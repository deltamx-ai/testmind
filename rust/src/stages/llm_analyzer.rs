...
use crate::types::{LLMOutput, LLMProviderKind, ResolvedLLMProvider, RiskLevel};
...
- 如果上下文提供了“相关知识库条目”或“需求与验收标准”，把它们视为项目级约束，不要忽略
...
额外要求：
- 如果输入里提供了“需求与验收标准”部分，必须逐条输出 requirementCoverage，不要遗漏
- 对 status=missing 或 partial 的需求，evidence 里要明确说明缺了什么
- 如果没有提供需求文本，requirementCoverage 返回空数组"#;

const USER_PROMPT_PREFIX: &str = "请分析以下代码变更，输出 JSON 格式的自测检查清单。\n\n";

pub async fn analyze_llm(context_text: &str, provider: &ResolvedLLMProvider) -> Result<LLMOutput> {
...
fn validate_output(parsed: &serde_json::Value) -> LLMOutput {
    ...
    let requirement_coverage = parsed
